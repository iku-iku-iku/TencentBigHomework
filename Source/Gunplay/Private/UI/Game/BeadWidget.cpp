...
=======
bool UBeadWidget::CanAdsorb(const FVector2D Coord)
{
	if (const AGunplayPlayerController* PlayerController = Cast<AGunplayPlayerController>(
		UGameplayStatics::GetPlayerController(this, 0)))
	{
		if (const APlayerCharacter* PlayerCharacter = PlayerController->GetPlayerCharacter())
		{
			if (const UHoldingComponent* HoldingComponent = PlayerCharacter->GetHoldingComponent())
			{
				if (const AActor* CurrentHolding = HoldingComponent->GetCurrentHolding())
				{
					if (const UShootComponent* ShootComponent = GunplayUtils::GetComponent_Const<UShootComponent>(
						CurrentHolding))
					{
						...
						FHitResult HitResult;
						if (ShootComponent->ShootDetect(Start, End, HitResult, PlayerCharacter))
						{
							if (const AActor* HitActor = HitResult.GetActor())
							{
								UClass* HitClass = HitActor->GetClass();
								return AdsorbClasses.ContainsByPredicate(
									[HitClass](const TSubclassOf<class AActor> AdsorbClass)
									{
										return HitClass->IsChildOf(AdsorbClass);
									});
							}
>>>>>>> 5a82f9e6a7647c355d223f9b8abdafe06ff475dc
						}
